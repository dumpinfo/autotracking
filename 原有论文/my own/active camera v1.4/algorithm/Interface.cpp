...
bool CInterface::GetState(CRECT *pRect)
{
	if (m_pProcess)
	{
		return m_pProcess->GetState(pRect);
	}
	return false;
}

bool CInterface::Start(int nWidth, int nHeight, HWND hWnd, CRECT InitRect,unsigned char * pInFrame)
{
	if(m_pProcess)
		return false;

	m_pProcess = new CProcess();
	return m_pProcess->Start(nWidth, nHeight, hWnd, InitRect, pInFrame);
}