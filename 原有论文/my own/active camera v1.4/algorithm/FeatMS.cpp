	...
	for (row = outbox.top+1; row <= outbox.bottom; row++)
	{
		...
		for (col = outbox.left+1; col <= outbox.right; col++)
		{
			if (row<=inRect.bottom && row>inRect.top && col>inRect.left && col<=inRect.right)
			{
				...
				for(i=0;i<FEATNUM;i++)
				{
					feat = floor((double)(m_nFeatList[i][0]*pTemp[2]+m_nFeatList[i][1]*pTemp[1]+m_nFeatList[i][2]*pTemp[0]+m_nFeatList[i][3])/m_nFeatList[i][4]);
...
bool CFeatMS::TrackNextFrame(unsigned char *pImage, CRECT inRect, TkResult *outResult)
{
	...
	outResult->score = min(1,countnonzero/double((rttemp.bottom - rttemp.top)*(rttemp.right-rttemp.left)));
	...
	for (row = outbox.top+1; row <= outbox.bottom; row++)
	{
		...
		for (col = outbox.left+1; col <= outbox.right; col++)
		{
			if (row<=inRect.bottom && row>inRect.top && col>inRect.left && col<=inRect.right)
			{
				...
				for(i=0;i<FEATNUM;i++)
				{
					feat = (int)floor((double)(m_nFeatList[i][0]*pTemp[2]+m_nFeatList[i][1]*pTemp[1]+m_nFeatList[i][2]*pTemp[0]+m_nFeatList[i][3])/m_nFeatList[i][4]);
					featbin = (int)floor(feat/8);
					//object histogram
					Pobj[i][featbin]++;
				}
