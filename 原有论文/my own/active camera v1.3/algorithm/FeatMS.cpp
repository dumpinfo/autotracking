...
/////在将来的实验中比较一下用float型和char进行计算的速度和精度的区别
	...
	memset(histObj, 0, sizeof(long)*FEATNUM*BINNUM);
	memset(histBg, 0, sizeof(long)*FEATNUM*BINNUM);
	...
	for (row = outbox.top+1; row <= outbox.bottom; row++)
	{
		pTemp = pImage + row*m_nWidthStep*3 + (outbox.left+1)*3;
		for (col = outbox.left+1; col <= outbox.right; col++)
		{
			if (row<=inRect.bottom && row>inRect.top && col>inRect.left && col<=inRect.right)
			{
				...
				for(i=0;i<FEATNUM;i++)
				{
					feat = floor((m_nFeatList[i][0]*pTemp[2]+m_nFeatList[i][1]*pTemp[1]+m_nFeatList[i][2]*pTemp[0]+m_nFeatList[i][3])/m_nFeatList[i][4]);
			...
			else
			{
				...
				for(i=0;i<FEATNUM;i++)
				{
					feat = ((m_nFeatList[i][0]*pTemp[2]+m_nFeatList[i][1]*pTemp[1]+m_nFeatList[i][2]*pTemp[0]+m_nFeatList[i][3])/m_nFeatList[i][4]);
					featbin = (int)floor(feat/8);
					//object histogram
					histBg[i][featbin]++;
				}
...
bool CFeatMS::TrackNextFrame(unsigned char *pImage, CRECT inRect, TkResult *outResult)
{
		...
		if (i==0)
		{
			...
			for (row = outbox.top; row<outbox.bottom; row++)
			{
				...
				for (col = outbox.left; col<outbox.right; col++)
				{
					...
					{
						countnonzero++;
						(*pTemp) = (unsigned char)(255.0*((*pTmp))/abs(maxratio));
					}
	...
	CRECT rttemp;
	rttemp.bottom = min(rowcenter + halfrow, m_nImgHeight);
	rttemp.top    = max(0, rowcenter - halfrow);
	rttemp.left   = max(0, colcenter - halfcol);
	rttemp.right  = min(colcenter + halfcol, m_nImgWidth);
	if (rowcenter>=m_nImgHeight || rowcenter<0 || colcenter>=m_nImgWidth || colcenter<0)
	{
		outResult->score = min(1,countnonzero/double(halfcol*halfcol*4));
	}
	else
	{
		outResult->score = min(1,countnonzero/double((rttemp.right - rttemp.left)*(rttemp.bottom-rttemp.top)));
	}
	outResult->occlusion	= (outResult->score<0.6? true:false);

	if (0==m_picnum && !(outResult->occlusion))
	{
...
{
	if (inRect.bottom<10 || inRect.top>m_nImgHeight-10 || inRect.left>m_nImgWidth-10 || inRect.right<10)
	{
		return;
	}	
	int halfwidth  = (inRect.right  - inRect.left)/2;
