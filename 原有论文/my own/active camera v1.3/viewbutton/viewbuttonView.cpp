...
//	}
	...
	if (m_bInitialBegin && !m_bInitialSet)
	{
		if (giClickCount>1)
		{
			dc.MoveTo(gaClickPoint[0]);
			for (int i = 1; i<giClickCount; i++)
			{
				dc.LineTo(gaClickPoint[i]);
				dc.MoveTo(gaClickPoint[i]);
			}
			dc.LineTo(gaClickPoint[0]);
		}
	}
	else if (m_bInitialBegin && m_bInitialSet)
	{
		CRECT Roi;
		
		if(m_pInterface->GetState(&Roi))
		{
//			Roi.bottom = m_Height - Roi.bottom;
//			Roi.top    = m_Height - Roi.top;
			dc.MoveTo(Roi.left+rtDisplay.left, Roi.top+rtDisplay.top);
			dc.LineTo(Roi.left+rtDisplay.left, Roi.bottom+rtDisplay.top);
			dc.MoveTo(Roi.left+rtDisplay.left, Roi.bottom+rtDisplay.top);
			dc.LineTo(Roi.right+rtDisplay.left, Roi.bottom+rtDisplay.top);
			dc.MoveTo(Roi.right+rtDisplay.left, Roi.bottom+rtDisplay.top);
			dc.LineTo(Roi.right+rtDisplay.left, Roi.top+rtDisplay.top);
			dc.MoveTo(Roi.right+rtDisplay.left, Roi.top+rtDisplay.top);
			dc.LineTo(Roi.left+rtDisplay.left, Roi.top+rtDisplay.top);
		}
	}
...
void CViewbuttonView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if (m_bPTZAuto && !m_bInitialSet && ( m_bCapture || m_bVideoFile))
	{
		CPaintDC dc(this); // device context for painting
		CRect rtDisplay = m_rtDisplay;
		int width       = rtDisplay.Width();
		...
		if (clickX< rtDisplay.left || clickX>rtDisplay.right || clickY<rtDisplay.top || clickY>=rtDisplay.bottom) return ; //x,y out of bound
		
		//get the click point coordinate		
		gaClickPoint[giClickCount].x = point.x;		
		gaClickPoint[giClickCount].y = point.y; // flip the click point for bmp processing				

		giClickCount++;
		m_bInitialBegin = true;

		if (giClickCount<4) return;		
		...
		RECT TEMP;
		GetRgnBox(gRegionCurrent, &TEMP);
		m_RectInit.bottom = TEMP.bottom - rtDisplay.top;
		m_RectInit.top  = TEMP.top - rtDisplay.top;
		m_RectInit.left = TEMP.left - rtDisplay.left;
		m_RectInit.right = TEMP.right - rtDisplay.left;
		if(m_pInFrame)
		{
			m_cVideo.ObtainFrame(m_pInFrame);
		}
		m_pInterface->Start(m_Width, m_Height, GetSafeHwnd(), m_RectInit, m_pInFrame);
	}
