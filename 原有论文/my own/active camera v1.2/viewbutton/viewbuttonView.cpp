...
void CViewbuttonView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_bPTZAuto && !m_bInitialSet && ( m_bCapture || m_bVideoFile))
	{
		...
		GetRgnBox(gRegionCurrent, &m_RectInit);
		m_RectInit.bottom -= rtDisplay.top;
		m_RectInit.top  -= rtDisplay.top;
		m_RectInit.left -= rtDisplay.left;
		m_RectInit.right -= rtDisplay.left;
		if(m_pInFrame)
		{
			m_cVideo.ObtainFrame(m_pInFrame);
		}
