...
void CButtonST::PreSubclassWindow() 
{
	...
	//SetBtnCursor(::LoadCursor(NULL, IDC_HAND) );
		//SetCursor(
		//IDC_APPSTARTING)
...
//		HRGN rEllipse= ::CreateEllipticRgn(0,0,rImage.right + 1,rImage.bottom + 1);

		::FillRect(hDC, &rRect, (HBRUSH)RGB(255, 255, 255));
//		::FillRgn(hDC, rEllipse, (HBRUSH)RGB(255, 255, 255));

//		::SetWindowRgn(GetSafeHwnd(), rEllipse,true);
	...
	else
	{
		HDC		hDC = NULL;
		CRgn rEllipse;
		rEllipse.CreateEllipticRgn(rImage.left,rImage.top,rImage.left+m_csBitmaps[byIndex].dwWidth ,rImage.top+m_csBitmaps[byIndex].dwHeight );
		if(m_bSETRGN)
			pDC->SelectClipRgn(&rEllipse,RGN_AND);
//		SetWindowRgn( rEllipse,true);
//		hDC = ::CreateCompatibleDC(pDC->m_hDC);
//		::FillRgn(hDC, rEllipse, (HBRUSH)RGB(255, 255, 255));
...
//		::BitBlt(pDC->m_hDC, rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, SRCPAINT);
		...
		pDC->BitBlt(rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, pDC->FromHandle(hdcMem), 0, 0, SRCAND);
		pDC->BitBlt(rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, pDC->FromHandle(hdcBmpMem), 0, 0, SRCPAINT);
//		pDC->FillRgn(&rEllipse,&m_brushForeground);
	} // else
...
//	DrawTextW(pDC->m_hDC ,p, -1, rpCaption, DT_WORDBREAK | DT_CENTER | DT_CALCRECT);
	...
	*/
	...
	//DrawTextW(pDC->m_hDC ,p, -1, rpCaption,DT_WORDBREAK | DT_CENTER);
		rpCaption->OffsetRect(-1, -1);
...
void CButtonST::SetRgn1(CRgn * rgn)
{
	if(m_pRgn)
	{
		delete m_pRgn;
...
void CButtonST::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	::SendMessage(this->GetParent()->GetSafeHwnd(),BUTTON_MESSAGE, wParam, 0);
}
...
void CButtonST::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	ReleaseCapture();
	m_bRunning = false;
	WPARAM wParam = (((DWORD)BUTTON_INACTIVE) << 16) + ((DWORD)GetDlgCtrlID());
	::SendMessage(this->GetParent()->GetSafeHwnd(),BUTTON_MESSAGE, wParam, 0);
}