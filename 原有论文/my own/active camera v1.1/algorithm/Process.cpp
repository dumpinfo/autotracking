...
UINT CProcess::WorkThread(LPVOID lpParameter)
{
	...
	while(!bEnd)
	{
		...
		if(nLastIndex!=curIndex)
		{
			...
			if (bKltFlag)
			{
				klt_TrackNextFrame(m_pInTemp, pThis->gAffineMatrix);

				//get zoom scale factor from affine matrix		
				pThis->zoom = (float)sqrt(fn_Abs(pThis->gAffineMatrix[0]*pThis->gAffineMatrix[4] - pThis->gAffineMatrix[1]*pThis->gAffineMatrix[3]));
				
				//if zoom scale is not 1 (different above threshold), accumulate zoom change.
				if (fabs(pThis->zoom-1)>0.002){		
					pThis->gZoomAccum = pThis->gZoomAccum*pThis->zoom;
				}

				//change size if not in predict mood		
				//change rect size based on scale factor accumulation
				pThis->gBlobWidth = (int)fn_Round(pThis->gBlobWidthInit * pThis->gZoomAccum);
				pThis->gBlobHeight = (int)fn_Round(pThis->gBlobHeightInit * pThis->gZoomAccum);

				//adjust the current Blob size

				utl_RectSizeAdjust(&Rect, pThis->gBlobWidth, pThis->gBlobHeight, pThis->m_WidthStep, pThis->m_Height);
			}
			else
			{
				for (int i=0;i<6;i++)
				{		
					pThis->gAffineMatrix[i] = 0;
				}
			}
			...
			if (fabs(tempx)<wlimit && fabs(tempy)<hlimit)
			{
				::PostMessage(pThis->m_hWnd,ALGORITHM_MESSAGE,0,0);
			}
			else if ( fabs(tempx) < fabs(tempy) )
			{
				if (tempy>0)
				{
					::PostMessage(pThis->m_hWnd,ALGORITHM_MESSAGE,2,0);//0:stop; 1:left; 2:up; 3:right; 4:down
				}
				else
				{
					::PostMessage(pThis->m_hWnd,ALGORITHM_MESSAGE,4,0);
				}
			}
			else
			{
				if (tempx>0)
				{
					::PostMessage(pThis->m_hWnd,ALGORITHM_MESSAGE,3,0);
				}
				else
				{
...
bool CProcess::Start(int nWidth, int nHeight, HWND hWnd, RECT InitRect, unsigned char * pInFrame)
{
	m_hWnd = hWnd;
	m_Width = nWidth;
	m_Height = nHeight;
...
bool CProcess::GetState(RECT *pRect)
{
	::EnterCriticalSection(&(m_CriticalSection));
	(*pRect) = m_RectState;
	::LeaveCriticalSection(&(m_CriticalSection));
